...
class HostCardEmulatorService: HostApduService() {
    ...
    companion object {
        ...
        val AID = "D00000D24F6331"
        val SELECT_INS = "A4"
        val DEFAULT_CLA = "00"
        val MIN_APDU_LENGTH = 18
    }
    ...
    override fun processCommandApdu(commandApdu: ByteArray?, extras: Bundle?): ByteArray {
        ...
        if (hexCommandApdu.substring(10, 24) == AID)  {
            PrefUtil.getSharedPreference(applicationContext)
            if (PrefUtil.getBoolean(PrefContracts.CARD_SAVED)){
                return Utils.hexStringToByteArray(PrefUtil.getString(PrefContracts.TAG_HEX_ID))
            }
